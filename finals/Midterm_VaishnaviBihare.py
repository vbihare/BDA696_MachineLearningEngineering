...
def main(file, response):
    ...
    for cols in condata:
        ...
        if response_var_type == "Boolean":
            file_path = (
                "finals/con_predictor_cat_response_distplot" + cols_proper + ".html"
            )
            ...
            file_path = (
                "finals/con_predictor_con_response_scatterplot" + cols_proper + ".html"
            )
    ...
    for cols in catdata:
        ...
        if response_var_type == "Boolean":
            file_path = (
                "finals/cat_predictor_cat_response_heatmap" + cols_proper + ".html"
            )
            ...
            file_path = (
                "finals/cat_predictor_con_response_violinplot" + cols_proper + ".html"
            )
    ...
    if not os.path.exists("finals/correlation"):
        os.makedirs("finals/correlation")
    ...
    corr1.savefig("finals/correlation/con_con_corr.png")
    plt.clf()

    with open("finals/correlation_con_con.html", "w") as _file:
    ...
    with open("finals/correlation/correlation_plot.html", "w") as _file:
        _file.write(
        ...
        msd_contcont = con_con_msd.sort_values(
            by="MeanSquareDiffWeighted", ascending=False
        )
        print(msd_contcont)

        # Cat-Cont Diff of mean
        # msd_catcont = con_con_msd.sort_values(by=
        # "MeanSquareDiffWeighted", ascending=False)
        # print(msd_catcont)
        #
        # # Cat-Cat
        # msd_catcat = cat_cat_msd.sort_values(by="
        # MeanSquareDiffWeighted", ascending=False)
        # print(msd_catcat)

        with open("finals/BruteForce.html", "w") as _file:
            _file.write(
...
def cat_or_con(predictor):
    if predictor.dtypes == "object" or predictor.nunique() < 5:
        return True
    else:
        return False
...
def get_msd(col1, col2, inp_data, pop_prop_1, t):
    ...
    filename = "finals/BruteForce_Plot_" + col1 + "_" + col2 + ".html"
    fig.write_html(
        file=filename,
        include_plotlyjs="cdn",
    )

    file_n = "BruteForce_Plot_" + col1 + "_" + col2 + ".html"
