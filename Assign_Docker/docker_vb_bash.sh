DATA=`mysqlshow -h assign_baseball_data -u root baseball | grep -v Wildcard | grep -o baseball`
...
# If the database doesn't exists then we will create the Baseball database
  ...
  mysql -h assign_baseball_data -u root -e "CREATE DATABASE IF NOT EXISTS baseball"
